...
const openai = new OpenAI({
  ...
  defaultHeaders: {
    "HTTP-Referer": "http://localhost:3000", // Replace with your actual site URL if deployed
    "X-Title": "AegntiX MVP" // Replace with your actual site title if deployed
  }
});
  ...
  async getAegntAction(aegntId: string, scenario: any) {
    ...
    try {
      ...
      aegnt.memory.push({
        timestamp: Date.now(),
        action,
        context: scenario.currentTime // Assuming scenario has a currentTime property
      });
      ...
      console.error("OpenRouter API Error:", error); // Log the actual error
      // Fallback for demo purposes
      return {
        aegntId,
        action: `${aegnt.role} considers the situation (API error or dummy response)`,
        timestamp: Date.now()
      };
    }
  }
