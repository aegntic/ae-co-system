...
export const LoginForm: React.FC<LoginFormProps> = ({ onSuccess, referralCode }) => {
  ...
  const handleProviderSignIn = async (provider: 'github' | 'google') => {
    ...
    try {
      ...
      // Success handling happens after redirect
    } catch (err) {
      setError(err instanceof Error ? err.message : 'An error occurred');
      setLoading(false);
    }
      ...
      <div className="glass-card p-8 space-y-6">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-400">
            {isSignUp 
              ? 'Start building amazing sites in seconds' 
        ...
        {referralCode && isSignUp && (
          <Alert 
            type="info" 
            message="You've been referred! You'll get 3 months of Pro features free after signing up." 
          />
        )}
          ...
          >
            {loading ? (
              <span className="flex items-center justify-center">
                <Icon name="loader-2" className="animate-spin mr-2" size={20} />
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            disabled={loading}
            className="flex items-center justify-center px-4 py-3 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
