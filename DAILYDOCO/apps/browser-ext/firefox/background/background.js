/**
 * DailyDoco Pro - Firefox Extension Background Script
 * Handles screen capture, desktop communication, and AI processing coordination
 * Adapted for Manifest V2 compatibility
 */
...
class DailyDocoBackground {
  ...
  init() {
    console.log('🚀 DailyDoco Pro background script starting...');
    ...
    console.log('✅ DailyDoco Pro background script ready');
  }
  ...
  async startCapture(options) {
    ...
    try {
      ...
      // Get media stream - Firefox uses different API
      const stream = await this.getMediaStreamFirefox(options.streamId);
      ...
      // Update badge - Firefox uses browserAction instead of action
  ...
  async stopCapture() {
    ...
    try {
      ...
      // Update badge - Firefox uses browserAction
  ...
  async getMediaStreamFirefox(streamId) {
    // Firefox uses getDisplayMedia for screen capture
    try {
      ...
      console.error('Firefox screen capture error:', error);
      throw error;
    }
  }
  ...
  // Additional methods from Chrome version...
