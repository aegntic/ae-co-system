...
def test_add_file_simple(temp_db_path, temp_file_with_content):
    ...
    cursor = db.execute("SELECT id, text, tags FROM PICKD")
    row = cursor.fetchone()
...
def test_add_file_with_tag_normalization(temp_db_path, temp_file_with_content):
    ...
    # Verify in database
    db = sqlite3.connect(temp_db_path)
    cursor = db.execute("SELECT id, tags FROM PICKD")
    row = cursor.fetchone()
...
def test_add_file_duplicate_id(temp_db_path, temp_file_with_content):
    ...
    db = sqlite3.connect(temp_db_path)
    cursor = db.execute("SELECT COUNT(*) FROM PICKD")
    count = cursor.fetchone()[0]
    assert count == 1
    
    cursor = db.execute("SELECT tags FROM PICKD WHERE id = ?", ("duplicate-file-id",))
