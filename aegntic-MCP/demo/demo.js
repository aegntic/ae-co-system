...
<body>
    ...
    <script>
        ...
        async function testIndividualServer(serverName) {
            const output = document.getElementById('demo-output');
            output.style.display = 'block';
            
            const displayName = serverName.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
            log(\`\\n🔧 Testing \${displayName}...\`);
            
            const serverTests = {
                'aegntic-knowledge-engine': async () => {
                    await testTool('aegntic-knowledge-engine', 'get_available_sources');
                    await testTool('aegntic-knowledge-engine', 'create_entities');
                    await testTool('aegntic-knowledge-engine', 'crawl_single_page');
                },
                'claude-export-mcp': async () => {
                    await testTool('claude-export-mcp', 'server_info');
                },
                'firebase-studio-mcp': async () => {
                    await testTool('firebase-studio-mcp', 'listProjects');
                },
                'n8n-mcp': async () => {
                    await testTool('n8n-mcp', 'listWorkflows');
            ...
            if (serverTests[serverName]) {
                await serverTests[serverName]();
                log(\`✅ \${displayName} testing completed!\`);
            } else {
                log(\`❌ Unknown server: \${serverName}\`);
            }
        }
    ...
    return Object.entries(this.servers).map(([name, config]) => {
      ...
      return `
        <div class="server-card">
          ...
          <button class="demo-button" onclick="testIndividualServer('${name}')">
            Test Server
          </button>
        </div>`;
    }).join('');
